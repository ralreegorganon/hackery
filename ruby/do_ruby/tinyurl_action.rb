#!/usr/bin/env ruby
%w{rubygems json trollop net/http do_ruby}.each { |lib| require lib}
include DoRuby
...
class TinyUrlAction
  ...
  def initialize()
    @action_definition = ActionDefinition.new :name => "Send to TinyUrl", :description => "Shortens a URL using TinyUrl"
    @return_items = []
  end
  ...
  def get_shortened_url_item(url)
    ...
    url_item = TextItem.new :name => "TinyUrl Item", :description => url, :text => tinyurl
  end
